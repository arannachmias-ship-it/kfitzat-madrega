...
export async function readJson(req, maxBytes = 8 * 1024) {
  // Vercel כבר מפענח גוף JSON לתוך req.body — ולכן בודקים את הגודל המוצהר לפני הכול
  const declared = Number(req.headers?.['content-length'] || 0);
  if (declared > maxBytes) throw Object.assign(new Error('body_too_large'), { status: 413 });
  if (req.body && typeof req.body === 'object') {
    if (Buffer.byteLength(JSON.stringify(req.body)) > maxBytes) throw Object.assign(new Error('body_too_large'), { status: 413 });
    return req.body;
  }
